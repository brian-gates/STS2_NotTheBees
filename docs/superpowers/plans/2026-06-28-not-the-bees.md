...
### Task 5: Scope the meme to Entomancer-created Dazed only

Goal: rename/reface only Dazed instances the Entomancer's `PersonalHivePower` creates, leaving all other Dazed vanilla. Adds a tag patch and gates the existing patches on the tag. This **replaces** the whole-file content from Task 3.

**Files:**
- Modify: `/Users/brian/code/bees/NotTheBeesMod.cs`

**Interfaces:**
- Consumes: `NotTheBeesMod.Initialize()`, the Title/Portrait prefixes (Tasks 2–3).
- Produces: `TagHiveCardPrefix(CardModel card)`, `CallerIsHivePower()`, `IsHiveDazed(CardModel)`, `_hiveDazed` (ConditionalWeakTable). Title/Portrait prefixes now gate on `IsHiveDazed` instead of bare `is Dazed`.

**Verified facts (from decompiling `sts2.dll` v0.107.1):**
- `PersonalHivePower.AfterDamageReceived` creates each Dazed and adds it via `CardPileCmd.AddGeneratedCardToCombat(CardModel card, PileType newPileType, Player? creator, CardPilePosition position)` (static async, single overload), called synchronously from the power's async state machine — so that frame is on the stack inside the prefix.
- Each in-combat Dazed is a fresh clone (`CombatState.CreateCard<T>` → `CreateCard(canonical, owner)` clones), so per-instance tagging is exact; the canonical/library Dazed is never tagged.

- [ ] **Step 1: Replace `NotTheBeesMod.cs` with the scoped version**
...
- [ ] **Step 3: Install and verify scoping in-game (deferred to human)**

Run `./install.sh`, launch STS2. Then:
- **Positive:** console → `fight ` + Tab to the Entomancer elite, fight it, take a powered-attack hit so it shuffles in Dazed. Those read **NOT THE BEES!** with the bee-cage art.
- **Negative:** console → `card DAZED` adds a non-hive Dazed; it must stay vanilla (normal name + art).
...
cd /Users/brian/code/bees
git add NotTheBeesMod.cs
git commit -m "feat: scope NOT THE BEES to Entomancer-created Dazed only"
```
